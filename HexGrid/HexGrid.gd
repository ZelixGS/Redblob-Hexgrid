...
func get_hex_by_position(position: Variant) -> Hex:
	assert(position is Vector2 or position is Vector3)
	var vector: Vector2
	if position is Vector2:
		vector = position
	if position is Vector3:
		vector = Vector2(position.x, position.z)
	return hex_round(pixel_to_hex(vector))

func get_hex_by_key(key: String) -> Hex:
...
func get_hex_in_radius(origin: Hex, N: int) -> Array[Hex]:
	...
	for q in range(-N, N +1):
		for r in range(max(-N, -q - N), min(N, -q + N) + 1):
			...
			var hex: Hex = get_hex_by_key(key)
			if hex:
				array.append(hex)
	return array
